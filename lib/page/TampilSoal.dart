...
class TampilSoal extends StatefulWidget {
  final String idSoal;
  const TampilSoal({super.key, required this.idSoal});

  @override
  State<TampilSoal> createState() => _TampilSoalState();
}

class _TampilSoalState extends State<TampilSoal> {
  ...
  void initState() {
    ...
    controller = WebViewController()
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onNavigationRequest: (NavigationRequest request) {
            if (request.url
                .startsWith('${VariableAPK.url2}tampilsoal/${widget.idSoal}')) {
      ...
      ..loadRequest(
          Uri.parse('${VariableAPK.url2}tampilsoal/${widget.idSoal}'));
  ...
  kirimText(BuildContext context) async {
    ...
    final response = await http.post(
        Uri.parse("${VariableAPK.url}apikirimtugastext"),
        body: {"jawab": _textController.text, "idTugas": widget.idSoal});
  ...
  kirimFile(BuildContext context) async {
    ...
    final response = await http
        .post(Uri.parse("${VariableAPK.url}apikirimtugasfile"), body: {
      "namafile": fileName,
      "file": base64Encode(baru!),
      "idTugas": widget.idSoal
    });
    final data = jsonDecode(response.body);
    String stat = data['status'];
    Fluttertoast.showToast(
