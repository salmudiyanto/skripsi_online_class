...
import 'package:smpn2parepare/widget/TugasCard.dart';
import 'package:smpn2parepare/widget/TugasListCard.dart';
...
class TugasList extends StatefulWidget {
  final String mapel;
  const TugasList({super.key, required this.mapel});

  @override
  State<TugasList> createState() => KuisScreenState();
}

class KuisScreenState extends State<TugasList> {
  bool load = true;
  List mapelJson = [];
  int idSiswa = 0;
  ...
  void getDataSoal() async {
    ...
    TugasSoalVM()
        .getTugasSoal(idSiswa.toString(), widget.mapel, 'B')
        .then((value) {
      setState(() {
        mapelJson = value;
        print(mapelJson);
  ...
  final List<String> tabBarItems = ["Tugas", "Nilai"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: kosongAppbar(context, "Tugas"),
        body: load
            ? Center(
...
class TugasListView extends StatelessWidget {
  ...
  const TugasListView({super.key, required this.mapelJson});
        ...
        child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return TugasListCard(
                ...
                id: mapelJson[index].idTugas.toString(),
              );
            }),
      ),
    );
